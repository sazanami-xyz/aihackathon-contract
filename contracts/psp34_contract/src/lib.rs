...
pub mod psp34_contract {
    ...
    impl Contract {
        ...
        pub fn mint(&mut self, id: Id) -> Result<(), PSP34Error> {
            // ink::env::debug_println!("hogehoge");
            self._mint_to(Self::env().caller(), id);
            // ink::env::debug_println!("aaaaa{:?}", self.psp34);
            // ink::env::debug_println!("bbbbb{:?}", self.metadata);
            Ok(())
        }
    ...
    pub mod tests {
        ...
        async fn return_collection_id_of_account(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            ...
            let address = client.instantiate("psp34_contract", &ink_e2e::alice(), constructor, 0, None)
                .await
